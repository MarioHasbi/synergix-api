...
exports.getAll = async (conditions) => {
  ...
  if (conditions.customer_name !== undefined) {
    customConditions.push(
      `customers.fullname like '%${conditions.customer_name}%'`
    );
  }
  ...
  if (conditions.phone_number !== undefined) {
    customConditions.push(
      `calls.phone_number like '%${conditions.phone_number}%'`
    );
  }
  ...
  if (conditions.start !== undefined) {
    customConditions.push(
      `quality_assurance_date BETWEEN '${conditions.start}'`
    );
...
exports.getDetail = async (conditions) => {
  ...
  const join = [
    ...
    `LEFT JOIN outbound_categories ON outbound_categories.id = calls.outbound_category_id`,
	`LEFT JOIN outbound_category_details ON outbound_category_details.id = calls.outbound_category_detail_id`
  ];
...
exports.insertManyData = async (data) => {
  const protectedColumns = ["id"];
  const cacheKeys = [table];
  const result = await dbQueryHelper.insertManyData({
    table,
    data,
    protectedColumns,
    cacheKeys,
