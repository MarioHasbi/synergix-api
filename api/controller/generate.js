...
exports.generateReport = async (conditions, report = 0) => {
    ...
    switch (report) {
        ...
        case '1':
            {
                dates = conditions.date !== undefined ? conditions.date : `${dateFormat(conditions.date, 'yyyy-mm-dd')}`
                title = "Report Call Activity"
                filename = 'report-call-activity'
                const header = [{ header: 'No', key: 'no' }, { header: 'Name', key: 'username' }, { header: 'ID', key: 'user_id' }, { header: "Login Time", key: 'login_time' }, { header: "Logout Time", key: 'logout_time' },
                { header: "Available Time", key: 'available_duration' }, { header: "Talk Time", key: 'talk_time' }, { header: "AUX Time - Rest Room", key: 'rest_room' }, { header: "AUX Time - Pray Time", key: 'pray_time' },
                { header: "AUX Time - Coaching Time", key: 'coaching_time' }, { header: "Break Time", key: 'break_time' }];
                const { data } = await callsController.getReportCallActivity(conditions)
                const timeColumn = [6, 7, 8, 9, 10, 11]
                const tableLine = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K']
                const mergeCells = 'A1:K2'
                const data_report = data

                generate = await excel.excel_generator(header, data_report, `${filename}-${dateFormat(conditions.date, 'yyyy-mm-dd')}`, dates, timeColumn, tableLine, title, mergeCells)
        ...
        case '2':
            {
                if (conditions.start !== undefined) {
                    dates = conditions.start + (conditions.end !== undefined ? " - " + conditions.end : "")
                }
                ...
                const { data } = await callsController.getReportPerformanceAgent(conditions)
                const header = [{ header: 'No', key: 'no' },
        ...
        case '3':
            {
                ...
                const header = [
                    ...
                    { header: 'Outbound Category Detail', key: 'outbound_categoriy_detail' },                   
                ...
                const { data } = await callsController.getReportCallCustomer(conditions)
                const data_report = data
        ...
        case '4':
            {
                ...
                const header = [
                    ...
                    { header: 'Campaign Name', key: 'campaign_name' },
                    { header: 'Number Customer Approved', key: 'number_customer_approved' },
                    { header: 'Nominal', key: 'nominal' },
                    { header: 'Average Customer Nominal Approved', key: 'average_customer_nominal_approved' }
                ];
                const { data } = await callsController.getBussinesssAchivement(conditions)
                const data_report = data
        ...
        case '6':
            {
                ...
                const card_number = `${data_report[0].card_number}` === "null" ? "" : data_report[0].card_number
                const QA_by = `${data_report[0].QA_by}`
                const agent = `${data_report[0].agent}`
                const qa_notes = `${data_report[0].qa_notes}`=== "null" ? "" : data_report[0].qa_notes
                ...
                generate = await excel.excel_generator_qa(header, data_report, `${filename}-${dateFormat(conditions.date, 'yyyy-mm-dd')}`, timeColumn, tableLine, title, mergeCells, mergeCells2,   calldate, durasi, customer_name, card_number, QA_by, agent, qa_notes, periods, total)
