...
const schema = require('../schema/break_histories')
const controller = require('../controller/break_histories')
...
router.get('/report-break', async (req,res,next) => {
    const conditions = req.query
    const result = await controller.getReportBreak(conditions)
    if (result.data === false) {
		return response.sendDataNotFound(res, result)
	}
