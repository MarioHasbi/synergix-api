 ...
 * @param  {Object.<string, string[]>} [conditionTypes={ 'like': [], 'date': [] }] - Condition option for certain table field. 'like' field will be treated with 'LIKE' condition. 'date' field will be treated iwht 'DATE()' function inside query condition
    ...
    return new Promise((resolve, reject) => {
        ...
        if (!_.isEmpty(conditions)) {
            ...
            Object.keys(conditions).forEach((fieldKey) => {
                if (!_.isEmpty(conditionTypes)) {
                    if (_.indexOf(conditionTypes.date, fieldKey) >= 0) {
                        let d = new Date()
                        let dateVal = (_.toNumber(conditions[fieldKey]) > 0) ? dateFormat(conditions[fieldKey] * 1000, 'yyyy-mm-dd') : dateFormat(d, 'yyyy-mm-dd')
                        setCond.push(`DATE(${table}.${fieldKey}) = ${dbConn.escape(dateVal)}`)
                    } else if (_.indexOf(conditionTypes.like, fieldKey) >= 0) {
                        let likeVal = `%${conditions[fieldKey]}%`
                        setCond.push(`${table}.${fieldKey} LIKE ${dbConn.escape(likeVal)}`)
                    } else {
                        setCond.push(`${table}.${fieldKey} = ${dbConn.escape(conditions[fieldKey])}`)
                    }
                } else {
                    setCond.push(`${table}.${fieldKey} = ${dbConn.escape(conditions[fieldKey])}`)
                }
 ...
 */
exports.getAll = ({ table = '', conditions = {}, conditionTypes = { 'like': [], 'date': [] }, customConditions = [], columnSelect = {}, columnDeselect = {}, customColumns = [], join = {}, groupBy = {}, having = {}, cacheKey = '' }) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (config.redis.serviceStatus == 1) {
...
exports.getDetail = ({ table = '', conditions = {}, columnSelect = [], columnDeselect = [], customColumns = [], join = [], cacheKey = '' }) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (config.redis.serviceStatus == 1 && table != '') {
            const key = cacheKey || table
            const getCache = await redisHelper.getData({ key: key, field: query })
